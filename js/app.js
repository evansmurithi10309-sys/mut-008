...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (pasteSampleBtn) {
    pasteSampleBtn.addEventListener('click', () => {
      const sampleMsg = "RKB91823X Confirmed. Ksh100.00 sent to Evans Mwenda 0715450987 on 16/9/26 at 5:30 PM. New M-PESA balance is Ksh1,250.00. Transaction cost, Ksh0.00.";
      if (mpesaMessageText) mpesaMessageText.value = sampleMsg;
      if (verificationErrorBox) verificationErrorBox.classList.add('hidden');
      showToast('📋 Sample M-PESA message pasted!', 'info');
    });
  }
  ...
  function verifyMpesaMessage(rawText) {
    ...
    // 1. Verify Amount: KES 100 (Visiting fee)
    const hasAmount = /(?:ksh|kes|sh|ksh\.)?\s*100(?:\.00)?/i.test(text) || lowerText.includes('100');
    if (!hasAmount) {
      return {
        isValid: false,
        error: 'Could not find visiting fee payment of KES 100 in the M-PESA message/screenshot. Please ensure the payment is exactly KES 100.'
      };
    }
  ...
  function runVerification() {
    ...
    if (!textToVerify && screenshotFileInput && screenshotFileInput.files && screenshotFileInput.files.length > 0) {
      showToast('Extracting screenshot text... If it takes too long, click "Paste Sample SMS" or paste text.', 'info');
    }
    ...
    // Save payment record in local storage
    DataStore.recordVisitingFeePayment(currentDetailHouseId, '0715450987', result.receiptCode);

    // Auto close modal after 1.8 seconds & show house details unlocked
    setTimeout(() => {
      closeModal('visiting-fee-modal');
      showToast(`🎉 M-PESA Payment Verified! KES 100 sent to Evans Mwenda (0715450987) confirmed. Landlord details unlocked!`, 'success');
